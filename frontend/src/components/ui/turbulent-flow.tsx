...
type TurbulentFlowProps = {
  children?: React.ReactNode;
};

export const Component = ({ children }: TurbulentFlowProps) => {
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        vec2 uv = vUv;
        vec3 pos = vec3(uv * u_noise_scale * 1.5, u_time * 0.05);
        float turb1 = turbulence(pos) * u_turbulence;
        float turb2 = turbulence(pos * 1.7 + vec3(100.0, 50.0, u_time * 0.3)) * u_turbulence * 0.3;
        float turb3 = fbm(pos * 0.8 + vec3(200.0, 100.0, u_time * 0.15)) * u_turbulence * 0.5;
        vec2 curlForce = curl(uv * 2.0, u_time * 0.5) * 0.25;
        vec2 distortion = vec2(turb1 + turb2, turb2 + turb3) * u_distortion + curlForce;
        vec2 distortedUV = uv + distortion;

        vec2 center1 = vec2(0.5 + sin(u_time * 0.4) * 0.3 + turb1 * 0.2, 0.75 + cos(u_time * 0.3) * 0.2 + turb2 * 0.3);
        vec2 center2 = vec2(0.75 + sin(u_time * 0.35) * 0.25 + turb2 * 0.8, 0.65 + cos(u_time * 0.45) * 0.3 + turb3 * 0.61);
        vec2 center3 = vec2(0.6 + sin(u_time * 0.5) * 0.2 + turb3 * 0.12, 0.25 + cos(u_time * 0.4) * 0.28 + turb1 * 0.09);
        vec2 center4 = vec2(0.15 + sin(u_time * 0.25) * 0.35 + turb1 * 0.11, 0.8 + cos(u_time * 0.55) * 0.22 + turb2 * 0.08);
        ...
        finalColor += color5 * (grad1 * grad2 * 0.7);
        finalColor += mix(color1, color2, 0.5) * (grad2 * grad3 * 0.93);
        finalColor += mix(color3, color4, 0.6) * (grad3 * grad4 * 0.35);

        float noiseDetail = fbm(vec3(uv * 15.0, u_time * 0.1)) * 0.5;
        float microTurbulence = turbulence(vec3(uv * 25.0, u_time * 0.05)) * 0.8;
